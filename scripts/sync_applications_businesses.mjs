...
const main = async () => {
  ...
  for (const doc of snap.docs) {
    ...
    const needsData = Object.keys(dataPatch).some((k) => {
      ...
      if (nv === null) return ov !== null && ov !== undefined;
      if (Array.isArray(nv)) return JSON.stringify(nv) !== JSON.stringify(ov || []);
      if (typeof nv === "object") return JSON.stringify(nv) !== JSON.stringify(ov || {});
      return String(nv) !== String(ov ?? "");
    });

    if (!needsStatus && !needsLink && !needsData) continue;
