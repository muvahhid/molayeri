...
export async function POST(request: Request) {
  ...
  if (action === 'update_user') {
    ...
    if (!userId) {
      return jsonNoStore({ error: 'Geçersiz userId.' }, { status: 400 })
    }
    ...
    if (body?.role !== undefined) {
      ...
      if (!ALLOWED_ROLES.has(role)) {
        return jsonNoStore({ error: 'Geçersiz rol değeri.' }, { status: 400 })
      }
      patch.role = role
    }
    ...
    if (body?.status !== undefined) {
      ...
      if (!ALLOWED_STATUSES.has(status)) {
        return jsonNoStore({ error: 'Geçersiz durum değeri.' }, { status: 400 })
      }
      patch.status = status
    }

    if (Object.keys(patch).length === 0) {
      return jsonNoStore({ error: 'Güncellenecek alan yok.' }, { status: 400 })
    }
    ...
    if (updateRes.error) {
      return jsonNoStore({ error: updateRes.error.message || 'Kayıt güncellenemedi.' }, { status: 500 })
    }

    if (!updateRes.data) {
      return jsonNoStore({ error: 'Kullanıcı bulunamadı.' }, { status: 404 })
    }
    ...
    return jsonNoStore({ ok: true, user: updateRes.data })
  }
  ...
  if (action === 'bulk_role') {
    ...
    const role = normalizeRole(body?.role)
    if (userIds.length === 0) {
      return jsonNoStore({ error: 'En az bir kullanıcı seçilmelidir.' }, { status: 400 })
    }
    if (!ALLOWED_ROLES.has(role)) {
      return jsonNoStore({ error: 'Geçersiz rol değeri.' }, { status: 400 })
    }
    ...
    for (const chunk of splitChunks(userIds, BULK_CHUNK_SIZE)) {
      ...
      if (res.error) {
        return jsonNoStore({ error: res.error.message || 'Toplu rol güncellemesi başarısız.' }, { status: 500 })
      }
    }
    ...
    await writeAdminAuditLog(ctx.adminSupabase, {
      ...
      action,
      targetTable: 'profiles',
      metadata: { affected: userIds.length, role },
      ...auditMeta,
    })

    return jsonNoStore({ ok: true, affected: userIds.length })
  }
  ...
  if (action === 'bulk_status') {
    ...
    for (const chunk of splitChunks(userIds, BULK_CHUNK_SIZE)) {
      ...
      if (res.error) {
        return jsonNoStore({ error: res.error.message || 'Toplu durum güncellemesi başarısız.' }, { status: 500 })
      }
    }
    ...
    await writeAdminAuditLog(ctx.adminSupabase, {
      ...
      action,
      targetTable: 'profiles',
      metadata: { affected: userIds.length, status },
      ...auditMeta,
    })
