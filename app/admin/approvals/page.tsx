...
const NeuCard = ({ children, className = "" }: { children: React.ReactNode, className?: string }) => (
  <div className={`bg-[#E0E5EC] rounded-[20px] shadow-[9px_9px_16px_rgb(163,177,198,0.6),-9px_-9px_16px_rgba(255,255,255,0.5)] ${className}`}>
    {children}
  </div>
)
  ...
  return (
    <div className="h-full flex flex-col font-sans text-slate-600">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end mb-6 gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-700">Onay Merkezi</h1>
              ...
              <tbody className="divide-y divide-slate-300/40">
                {filteredList.map(biz => (
                  <tr key={biz.id} onClick={() => openDetail(biz)} className="group cursor-pointer hover:bg-slate-200/50">
                    <td className="py-4 pl-4 font-black text-slate-700 group-hover:text-blue-600 transition-colors">{biz.name}</td>
      ...
      {selectedBiz && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-[#E0E5EC] w-full max-w-6xl h-[90vh] flex flex-col rounded-[40px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300 border border-white/40">
            <div className="flex justify-between items-center p-6 border-b border-slate-300/50 bg-[#E0E5EC]">
              <div className="flex items-center gap-3"><h2 className="text-2xl font-black text-slate-700">{selectedBiz.name}</h2></div>
              ...
              {detailLoading ? <div className="h-full flex items-center justify-center"><Loader2 className="animate-spin text-blue-500 w-10 h-10"/></div> : (
                <div className="grid grid-cols-12 gap-8">
                  ...
                  <div className="col-span-12 lg:col-span-8 space-y-6">
                    ...
                    {/* Görsel Alanı */}
                    <div className="bg-[#E0E5EC] p-6 rounded-[30px] shadow-sm">
                       {activeImage ? (
                       ...
                       {selectedBiz.gallery && selectedBiz.gallery.length > 0 && (
                         <div className="mt-4">
                            <h4 className="text-[10px] font-black text-slate-400 mb-2 uppercase flex items-center gap-1"><Layers className="w-3 h-3"/> DİĞER RESİMLER ({selectedBiz.gallery.length})</h4>
                       ...
                       <h3 className="text-sm font-black text-slate-400 mb-2 mt-6 uppercase tracking-widest">AÇIKLAMA</h3>
                       <p className="text-slate-600 text-sm leading-relaxed">{selectedBiz.description || 'Açıklama girilmemiş.'}</p>
                    </div>

                    <div className="grid grid-cols-2 gap-6">
                       <div className="bg-[#E0E5EC] p-6 rounded-[30px] shadow-sm">
                          <h3 className="text-xs font-black text-slate-400 mb-4 uppercase tracking-widest flex items-center gap-2"><MapPin className="w-4 h-4"/> ADRES</h3>
                       ...
                       <div className="bg-[#E0E5EC] p-6 rounded-[30px] shadow-sm">
                          <h3 className="text-xs font-black text-slate-400 mb-4 uppercase tracking-widest flex items-center gap-2"><Phone className="w-4 h-4"/> İLETİŞİM</h3>
                          <p className="text-lg font-black text-slate-700">{selectedBiz.phone}</p>
                       </div>
                    </div>

                    <div className="bg-[#E0E5EC] p-6 rounded-[30px] shadow-sm">
                       <h3 className="text-xs font-black text-slate-400 mb-4 uppercase tracking-widest">SEÇİLEN ÖZELLİKLER</h3>
                  ...
                  <div className="col-span-12 lg:col-span-4 space-y-6">
                    <div className="bg-[#E0E5EC] p-6 rounded-[30px] shadow-sm border border-slate-300/50">
                       <h3 className="text-xs font-black text-slate-400 mb-4 uppercase tracking-widest flex items-center gap-2"><User className="w-4 h-4"/> BAŞVURAN</h3>
                       {ownerDetails ? (
                         <div className="text-center">
                            ...
                            <div className="font-black text-slate-700">{ownerDetails.full_name || 'İsimsiz'}</div>
                    ...
                    <div className="bg-white p-6 rounded-[30px] shadow-lg border border-white sticky top-0">
                       <h3 className="text-xs font-black text-slate-400 mb-6 uppercase tracking-widest text-center">KARARINIZ NEDİR?</h3>
                       <div className="space-y-3">
                          <button onClick={() => handleDecision(selectedBiz.id, 'active')} className="w-full py-4 bg-green-500 hover:bg-green-600 text-white rounded-2xl font-black shadow-lg shadow-green-200 transition-all active:scale-95 flex items-center justify-center gap-2"><CheckCircle className="w-5 h-5"/> ONAYLA</button>
                          <button onClick={() => handleDecision(selectedBiz.id, 'rejected')} className="w-full py-4 bg-red-500 hover:bg-red-600 text-white rounded-2xl font-black shadow-lg shadow-red-200 transition-all active:scale-95 flex items-center justify-center gap-2"><XCircle className="w-5 h-5"/> REDDET</button>
                       </div>
                    </div>
                  </div>
