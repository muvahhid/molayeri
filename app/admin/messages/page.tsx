...
function MessagesContent() {
  ...
  const handleReply = (msg: any) => {
    ...
    setSubject(`Ynt: ${msg.subject}`)
    setContent(`\n\n--- ${new Date(msg.created_at).toLocaleDateString()} tarihinde yazıldı: ---\n> ${msg.content}\n`)
  }
                    ...
                    >
                        <div className="flex justify-between items-start mb-3">
                           <div className="flex items-center gap-4">
                              <div className={`w-12 h-12 rounded-2xl ${BG_COLOR} flex items-center justify-center text-blue-500 shadow-md border border-white overflow-hidden`}>
        ...
        {activeTab === 'compose' && (
          <div className="h-full flex flex-col lg:flex-row gap-8 animate-in fade-in slide-in-from-right-4 duration-500">
            <div className="flex-1 flex flex-col gap-6">
              <div className="flex gap-4">
                 {[{ id: 'broadcast_all', icon: Users, label: 'TÜM ÜYELER' }, { id: 'broadcast_business', icon: Store, label: 'İŞLETMELER' }, { id: 'single', icon: User, label: 'TEK KİŞİ' }].map((t) => (
                   <NeuButton key={t.id} onClick={() => setTargetType(t.id as any)} variant={targetType === t.id ? 'active' : 'primary'} className="flex-1 py-4 text-xs"><t.icon className="w-5 h-5"/> {t.label}</NeuButton>
                 ))}
              </div>
              {targetType === 'single' && (
              ...
              <div className="space-y-4 flex-1 flex flex-col">
                 ...
                 <div className={`flex-1 flex flex-col rounded-2xl ${BG_COLOR} ${SHADOW_IN} overflow-hidden border border-white/50 relative`}>
                    <div className="flex items-center gap-1 p-3 border-b border-slate-200/50 bg-white/20 backdrop-blur-sm">
                       ...
                       <button onClick={() => setPreviewMode(!previewMode)} className={`text-xs font-bold px-3 py-1.5 rounded-lg flex items-center gap-2 transition-colors ${previewMode ? 'bg-blue-100 text-blue-600' : 'text-slate-500 hover:bg-white'}`}><Eye className="w-4 h-4"/> {previewMode ? 'Düzenle' : 'Önizle'}</button>
                    </div>
                    <div className="flex-1 relative">
                       ...
                       ) : ( <textarea ref={textareaRef} className={`w-full h-full bg-transparent p-6 text-slate-700 font-medium text-sm outline-none resize-none leading-relaxed`} placeholder="Mesajınızı buraya yazın..." value={content} onChange={(e) => setContent(e.target.value)}></textarea> )}
                    </div>
                 </div>
              </div>
            </div>
            <div className="w-full lg:w-80 flex flex-col gap-6">
              ...
              <NeuButton onClick={handleSend} variant="solid" className="py-5 text-lg shadow-xl shadow-blue-500/20" disabled={sending}>{sending ? <Loader2 className="animate-spin w-6 h-6"/> : <><Send className="w-5 h-5"/> GÖNDER</>}</NeuButton>
              <div className="text-[10px] text-slate-400 text-center leading-relaxed px-4">Toplu gönderimler yoğunluğa göre sırayla iletilir. Spam kurallarına uyunuz.</div>
            </div>
          </div>
        )}
      </div>
                 ...
                 <div className="flex gap-4">
                    ...
                    <div>
                       ...
                       <div className="flex items-center gap-3 mt-2 text-xs font-bold text-slate-400">
                          ...
                          <span className="flex items-center gap-1"><Clock className="w-3 h-3"/> {new Date(selectedMessage.created_at).toLocaleTimeString()}</span>
                       </div>
                    </div>
                 </div>
