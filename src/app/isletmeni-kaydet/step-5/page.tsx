...
const PROXY_BASE = (process.env.NEXT_PUBLIC_PLACES_PROXY_URL || "").replace(
  /\/$/,
  "",
);
...
export default function Step5OzetBasvuruPage() {
    ...
    try {
      const uploadSessionId =
        typeof window !== "undefined"
          ? window.localStorage.getItem("molayeri_upload_session")
          : null;
      // --- ROAD SNAP + ROAD TYPE (proxy) ---
      let roadLat: number | null = null;
      let roadLng: number | null = null;
      let roadPlaceId: string | null = null;
      let roadType: string | null = null;

      try {
        const lat = state.business.lat ?? null;
        const lng = state.business.lng ?? null;
        if (typeof lat === "number" && typeof lng === "number" && PROXY_BASE) {
          const r1 = await fetch(
            `${PROXY_BASE}/roads/nearest?lat=${lat}&lng=${lng}`,
          );
          const j1 = await r1.json();
          roadLat = typeof j1?.roadLat === "number" ? j1.roadLat : null;
          roadLng = typeof j1?.roadLng === "number" ? j1.roadLng : null;
          roadPlaceId =
            typeof j1?.roadPlaceId === "string" ? j1.roadPlaceId : null;

          const r2 = await fetch(
            `${PROXY_BASE}/osmr/type?lat=${lat}&lng=${lng}`,
          );
          const j2 = await r2.json();
          roadType = typeof j2?.roadType === "string" ? j2.roadType : null;
        }
      } catch {
        // sessiz geç: kayıt yine de devam etsin
      }
      ...
      const payload = {
        ...
        photos: (photos || [])
          .map((p) => ({ url: p.url, isCover: !!p.isCover }))
          .slice(0, 6),
        uploadSessionId,
      };
  ...
  React.useEffect(() => {
    ...
    try {
      const rawW =
        typeof window !== "undefined"
          ? window.localStorage.getItem("molayeri_wizard_v1")
          : null;
    ...
    (async () => {
      try {
        ...
        const urls = await Promise.all(
          res.items.map((it) => getDownloadURL(it)),
        );
        if (!alive) return;
        const list = urls
          .slice(0, 6)
          .map((u, i) => ({ url: u, isCover: i === 0 }));
        setPhotos(list);
      } catch {
        if (alive) setPhotos([]);
      } finally {
  ...
  return (
    <div className="px-6 pb-6">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <div className="rounded-[22px] border border-white/10 bg-[#151A24] p-5">
          ...
          <div className="mt-3 space-y-2 text-sm text-white/70">
            <div>
              <span className="text-white/45">İsim:</span>{" "}
              {state.user.firstName} {state.user.lastName}
            </div>
            <div>
              <span className="text-white/45">Email:</span> {state.user.email}
            </div>
            <div>
              <span className="text-white/45">Telefon:</span> {state.user.phone}
            </div>
        ...
        <div className="rounded-[22px] border border-white/10 bg-[#151A24] p-5">
          ...
          <div className="mt-3 space-y-2 text-sm text-white/70">
            ...
            <div>
              <span className="text-white/45">Adres:</span>{" "}
              {state.business.addressText || "-"}
            </div>
            <div>
              <span className="text-white/45">Tanıtım:</span>{" "}
              {state.business.description || "-"}
            </div>
            <div>
              <span className="text-white/45">Yol:</span>{" "}
              {state.business.roadName || state.business.roadNote
                ? `${state.business.roadName || "-"} • ${state.business.roadNote || "-"}`
                : "-"}
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between">
              ...
              <div className="text-xs text-white/60">
                {loadingPhotos ? "Yükleniyor…" : `${photos.length}/6`}
              </div>
            </div>
            ...
            {photos.length > 0 ? (
              <div className="mt-3 grid grid-cols-3 gap-3 sm:grid-cols-6">
                {photos.slice(0, 6).map((p, i) => (
                  <div
                    key={i}
                    className="overflow-hidden rounded-xl border border-white/10 bg-[#151A24]"
                  >
                    <img
                      src={p.url}
                      alt=""
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        <Button
          variant="primary"
          className="px-8 py-4 rounded-[22px]"
        ...
        footer={
          <div className="flex justify-end gap-2">
            <Button
              ...
              onClick={() => {
                setOpen(false);
                if (userRole === "pending" || !userRole) router.push("/login");
              }}
            >
              Tamam
            </Button>
