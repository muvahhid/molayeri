  ...
  try {
    ...
    for (const k of Object.keys(rawFV)) {
      ...
      if (t === "bool") fv[k] = { type: "bool", value: safeBool(x?.value) };
        ...
        fv[k] = { type: "text", value: safeString(x?.value) };
      else if (t === "select")
        fv[k] = { type: "select", value: safeString(x?.value) };
      else if (t === "multiSelect")
        fv[k] = { type: "multiSelect", value: safeStrArr(x?.value) };
    }
    ...
    return {
      ...
      business: {
        ...
        roadNote: safeString(obj?.business?.roadNote),
        roadCodes: safeStrArr(obj?.business?.roadCodes),
      },
      selectedCategoryIds: selected,
      activeCategoryId: selected.includes(active)
        ? active
        : selected[0] || null,
      featureValues: fv,
    };
    ...
    () => ({
      ...
      toggleCategory: (categoryId, on) =>
        setState((prev) => {
          ...
          if (!on && nextActive === categoryId)
            nextActive = nextSelected[0] || null;
          return {
            ...prev,
      ...
      setFeatureValue: (featureId, v) =>
        setState((prev) => ({
          ...prev,
          featureValues: { ...prev.featureValues, [featureId]: v },
        })),
      reset: () => {
